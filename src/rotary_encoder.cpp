...
namespace tarefa3::rotary_encoder
{
  ...
  RotaryEncoder::RotaryEncoder(
      uint8_t output_a_pin_,
      uint8_t output_b_pin_, uint8_t switch_pin_,
      const RotaryEncoderPinsManager *pins_manager_)
      : output_a_pin(output_a_pin_),
        output_b_pin(output_b_pin_),
        switch_pin(switch_pin_),
        pins_manager(pins_manager_)
  {
  ...
  {
    return this->previous_state->output_a_state != this->current_state->output_a_state;
  }

  bool RotaryEncoder::switch_has_been_pressed()
  {
    return this->previous_switch_state && !this->current_switch_state;
  }

  bool RotaryEncoder::check_if_rotation_is_clockwise()
  {
    if (this->previous_state->output_a_state != this->current_state->output_a_state)
    {
      return this->current_state->output_b_state == this->current_state->output_a_state;
    }
  ...
  void RotaryEncoder::update()
  {
    ...
    this->current_switch_state = this->pins_manager->get_rotary_output_digital_level(this->switch_pin);
  }
  ...
  RotationReport RotaryEncoder::get_report()
  {
    ...
    RotationReport report = {
        ...
        .switch_state_has_changed = this->previous_switch_state != this->current_switch_state,
